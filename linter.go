...
func filterIssues(patchFile string, issues []result.Issue) ([]result.Issue, error) {
	buf, err := ioutil.ReadFile(patchFile)
	if err != nil {
		return nil, err
	}
	m, err := linesChanged(buf)
	if err != nil {
		return nil, err
	}
...
func linesChanged(s []byte) (map[string][]pos, error) {
	diff, err := diffparser.Parse(string(s))
	if err != nil {
		return nil, err
	}

	dFiles := make(map[string][]pos)
		...
		for _, h := range f.Hunks {
			for _, dl := range h.NewRange.Lines {
				if dl.Mode == diffparser.ADDED {
					dFiles[f.NewName] = append(dFiles[f.NewName], pos{dl.Number, dl.Position})
				}
			}
		}
	}
